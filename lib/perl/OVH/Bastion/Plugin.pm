...
# validate user/host/ip/port when specified, undef them otherwise (instead of '')
# this sets the vars user/host/ip/port exported to the plugins, called in begin() below.
# can also be called by the plugins themselves to change/validate the user/host/ip/port vars,
# from arguments they might have received themselves.
# when init=1, make the assumption than user/host/ip/port are already set and just validate them
sub validate_tuple {
    my %params             = @_;
